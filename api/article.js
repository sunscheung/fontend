...
router.post("/type/html", (req, res) => {
  ...
  HTML5Model.findOne({ title }).then(article => {
    if (article) {
      res.send(article);
    } else {
...
router.post("/type/css", (req, res) => {
  const title = req.body.title;
  CSSModel.findOne({ title }).then(article => {
    if (article) {
      res.send(article);
    } else {
...
router.get("/javascript", (req, res) => {
  JavaScriptModel.find().then(javascript => {
    res.send(javascript);
  });
});
...
router.post("/type/javascript", (req, res) => {
  const title = req.body.title;
  JavaScriptModel.findOne({ title }).then(article => {
    if (article) {
      res.send(article);
    } else {
      res.status(404).send("文章不存在");
...
router.get(
  ...
  (req, res) => {
    var allArticles = [];
    // 查询所有文章
    HTML5Model.find().then(html5s => {
      allArticles = html5s;

      CSSModel.find().then(csss => {
        for (let i = 0; i < csss.length; i++) {
          allArticles[allArticles.length] = csss[i];
        }

        JavaScriptModel.find().then(javascripts => {
...
router.post("/search", (req, res) => {
  ...
  const result = [];
  HTML5Model.find({ content: { $regex: search, $options: "i" } }).then(
    article => {
      if (article.length) {
      ...
      CSSModel.find({ content: { $regex: search, $options: "i" } }).then(
        csss => {
          if (csss.length) {
            csss.forEach(item => {
              result[result.length] = item;
