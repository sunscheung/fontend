...
router.post("/register", (req, res) => {
  ...
  User.findOne({ email }).then(user => {
    if (user) {
      ...
      // 用户注册日志
      setTimeout(() => {
        UserInfo.findOne({ email }).then(userid => {
          ...
          try {
            fs.writeFileSync(
              path.resolve(
                __dirname,
                `../static/user/${userid._id}/default.png`
              ),
              fs.readFileSync(
                path.resolve(__dirname, `../static/avatar/default.png`)
              )
            );
            console.log("头像初始化成功");
          } catch (error) {
  ...
  User.findOne({ email }).then(user => {
    if (!user) {
      ...
      if (!isValidPassword) {
        ...
        UserInfo.findOne({ email }).then(userinfo => {
          Course.findOne({ email }).then(usercourse => {
            // 用户登录日志
      ...
      UserInfo.findOne({ email }).then(userinfo => {
        Course.findOne({ email }).then(usercourse => {
          ...
          if (req.body.signdate) {
            ...
            rule.signdate = userinfo.signdate;
          }
          // 签到次数
          if (req.body.signcount) {
            rule.signcount = req.body.signcount;
          } else {
            rule.signcount = userinfo.signcount;
          }
          // 头像
          if (req.body.avatar) {
            rule.avatar = req.body.avatar;
          } else {
            rule.avatar = userinfo.avatar;
          }
          // 电话
          if (req.body.phone) {
            rule.phone = req.body.phone;
          } else {
            rule.phone = userinfo.phone;
          }
          // 余额
          if (req.body.money) {
            rule.money = req.body.money;
          } else {
            rule.money = userinfo.money;
          }
          // 是否购买html5
          if (req.body.html5) {
          ...
          if (req.body.javascript) {
            ...
            rule.javascript = usercourse.javascript;
          }
          // 购买日期
          if (req.body.startdate) {
            rule.startdate = req.body.startdate;
          } else {
            rule.startdate = usercourse.startdate;
          }
          // 到期日期
          if (req.body.enddate) {
            rule.enddate = req.body.enddate;
          } else {
            rule.enddate = usercourse.enddate;
          }
          // 购买次数
          if (req.body.buycount) {
            rule.buycount = req.body.buycount;
          } else {
            rule.buycount = usercourse.buycount;
          }
...
router.get("/avatar", (req, res) => {
  ...
  UserInfo.findOne({ email }).then(user => {
    if (user) {
      res.sendFile(
        path.resolve(__dirname, `../static/user/${user._id}/${user.avatar}`)
      );
    } else {
...
router.get("/avatar/back", (req, res) => {
  ...
  UserInfo.findOne({ email }).then(user => {
    if (user) {
      res.sendFile(
        path.resolve(__dirname, `../static/user/${user._id}/${avatar}`)
      );
    }
  });
});
