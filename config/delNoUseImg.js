...
module.exports = (flag, className) => {
  ...
  if (flag == 4) {
    vipHTML.find().then(htmls => {
      ...
      allPoster.forEach(poster => {
        ...
        if (!isexist) {
          fs.unlinkSync(
            path.resolve(__dirname, "../static/media/html/" + poster)
          );
      ...
      allVideos.forEach(video => {
        ...
        if (!isexist) {
          fs.unlinkSync(
            path.resolve(__dirname, "../static/media/html/" + video)
          );
  ...
  if (flag == 5) {
    vipCSS.find().then(csss => {
      ...
      allPoster.forEach(poster => {
        ...
        if (!isexist) {
          fs.unlinkSync(
            path.resolve(__dirname, "../static/media/css/" + poster)
          );
      ...
      allVideos.forEach(video => {
        ...
        if (!isexist) {
          fs.unlinkSync(
            path.resolve(__dirname, "../static/media/css/" + video)
          );
  ...
  if (flag == 6) {
    vipJS.find().then(jss => {
      ...
      allPoster.forEach(poster => {
        ...
        if (!isexist) {
          fs.unlinkSync(
            path.resolve(__dirname, "../static/media/js/" + poster)
          );
  ...
  if (flag == 2) {
    userInfo.findOne({ _id: className }).then(user => {
      files.forEach(item => {
        var isexist = false;
        if (item != user.avatar) {
          isexist = true;
